...
// Single-use here — a shared component would be premature for one call
// site. "Type a tag and press Tab" per the reference; Enter also works,
// since requiring literally Tab (which usually moves focus) is an odd UX
// expectation to hold a user to.
function TagInput({ tags, onChange }: { tags: string[]; onChange: (next: string[]) => void }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 rounded-lg border border-black/15 dark:border-white/15 bg-white dark:bg-zinc-900 px-2 py-1.5 min-h-9 focus-within:border-accent focus-within:ring-[3px] focus-within:ring-accent/20">
      {tags.map((tag) => (
        <span
          key={tag}
          className="flex items-center gap-1 rounded-full bg-black/5 dark:bg-white/10 px-2 py-0.5 text-xs"
        >
          {tag}
          <button
            type="button"
