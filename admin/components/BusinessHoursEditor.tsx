    ...
    <div className="space-y-4">
      {WEEKDAYS.map((day) => (
        <div key={day} className="flex flex-wrap items-center gap-x-3 gap-y-1">
          <span className="w-24 shrink-0 text-sm text-text-secondary dark:text-zinc-400">{WEEKDAY_LABELS[day]}</span>
          {!value[day].closed && (
            <div className="flex shrink-0 items-center gap-2">
              <input
                ...
                className="h-8 rounded-lg border border-border dark:border-white/15 bg-surface dark:bg-zinc-900 px-2 text-sm outline-none transition-shadow focus:border-accent focus:ring-[3px] focus:ring-accent/20"
              />
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <span className="w-12 text-sm text-text-muted dark:text-zinc-400">
