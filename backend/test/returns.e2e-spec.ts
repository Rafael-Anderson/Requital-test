...
describe('Order Returns/Refunds (e2e)', () => {
  ...
  describe('refund calculation', () => {
    it('computes the default refund from priceAtPurchase * returned quantity, editable via override', async () => {
      const { adminToken, categoryId, outletId } = await setupShop('return-calc');
      const product = await createProduct(adminToken, categoryId, { price: 25 });
      await seedStock(adminToken, outletId, product.id, 10);
      const order = await createDeliveredOrder(adminToken, outletId, product.id, 4);
      ...
      const res = await request(app.getHttpServer())
        ...
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ items: [{ orderItemId: itemId, quantity: 2 }], reason: 'damaged', restock: false })
        .expect(201);
    ...
    it('rejects returning more units of a line item than remain unreturned', async () => {
      const { adminToken, categoryId, outletId } = await setupShop('return-line-cap');
      const product = await createProduct(adminToken, categoryId);
      await seedStock(adminToken, outletId, product.id, 10);
      const order = await createDeliveredOrder(adminToken, outletId, product.id, 3);
      const itemId = order.orderitem[0].id;
  ...
  describe('running-total cap', () => {
    it('never lets cumulative refunds across multiple returns exceed the order total', async () => {
      const { adminToken, categoryId, outletId } = await setupShop('return-running-cap');
      const product = await createProduct(adminToken, categoryId, { price: 30 });
      await seedStock(adminToken, outletId, product.id, 10);
      const order = await createDeliveredOrder(adminToken, outletId, product.id, 3); // total 90
      const itemId = order.orderitem[0].id;
      ...
      await request(app.getHttpServer())
        .post(`/orders/${order.id}/returns`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ items: [{ orderItemId: itemId, quantity: 2 }], reason: 'damaged', restock: false, refundAmount: 80 })
        .expect(201);
      ...
      const res = await request(app.getHttpServer())
        .post(`/orders/${order.id}/returns`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ items: [{ orderItemId: itemId, quantity: 1 }], reason: 'damaged', restock: false, refundAmount: 20 })
        .expect(400);
    ...
    it('flips order.paymentStatus to refunded once cumulative refunds reach the total', async () => {
      const { adminToken, categoryId, outletId } = await setupShop('return-full-refund');
      const product = await createProduct(adminToken, categoryId, { price: 40 });
      await seedStock(adminToken, outletId, product.id, 10);
      const order = await createDeliveredOrder(adminToken, outletId, product.id, 2); // total 80
      const itemId = order.orderitem[0].id;

      await request(app.getHttpServer())
        .post(`/orders/${order.id}/returns`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ items: [{ orderItemId: itemId, quantity: 2 }], reason: 'changed_mind', restock: false })
        .expect(201);
  ...
  describe('restock', () => {
    it('restocking a return increments stock and logs a RETURN stockmovement row', async () => {
      const { adminToken, categoryId, outletId } = await setupShop('return-restock');
      const product = await createProduct(adminToken, categoryId);
      await seedStock(adminToken, outletId, product.id, 10);
      const order = await createDeliveredOrder(adminToken, outletId, product.id, 3);
      ...
      await request(app.getHttpServer())
        .post(`/orders/${order.id}/returns`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ items: [{ orderItemId: itemId, quantity: 2 }], reason: 'wrong_item', restock: true })
        .expect(201);
    ...
    it('does not restock when restock is false', async () => {
      const { adminToken, categoryId, outletId } = await setupShop('return-no-restock');
      const product = await createProduct(adminToken, categoryId);
      await seedStock(adminToken, outletId, product.id, 10);
      const order = await createDeliveredOrder(adminToken, outletId, product.id, 2);
      ...
      await request(app.getHttpServer())
        .post(`/orders/${order.id}/returns`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ items: [{ orderItemId: itemId, quantity: 1 }], reason: 'damaged', restock: false })
        .expect(201);
  ...
  describe('provider refund fallback', () => {
    it('falls back to a manual refund and still records it when the provider call fails', async () => {
      const { adminToken, categoryId, outletId } = await setupShop('return-provider-fail');
      const product = await createProduct(adminToken, categoryId);
      await seedStock(adminToken, outletId, product.id, 10);
      const order = await createDeliveredOrder(adminToken, outletId, product.id, 2);
      const itemId = order.orderitem[0].id;
  ...
  describe('eligibility', () => {
    it('rejects a return for an order that is not delivered', async () => {
      const { adminToken, categoryId, outletId } = await setupShop('return-not-delivered');
      const product = await createProduct(adminToken, categoryId);
      ...
      await request(app.getHttpServer())
        .post(`/orders/${order.id}/returns`)
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ items: [{ orderItemId: order.orderitem[0].id, quantity: 1 }], reason: 'damaged' })
        .expect(400);
    });
  });
  ...
  describe('tenant isolation (adversarial)', () => {
    it("cannot create or view returns on another shop's order", async () => {
      ...
      const orderA = await createDeliveredOrder(shopA.adminToken, shopA.outletId, productA.id, 2);

      await request(app.getHttpServer())
        .post(`/orders/${orderA.id}/returns`)
        .set('Authorization', `Bearer ${shopB.adminToken}`)
        .send({ items: [{ orderItemId: orderA.orderitem[0].id, quantity: 1 }], reason: 'damaged' })
        .expect(404);
