...
// valid staff token for shop A must never unlock shop B's unpublished
// content, since previewToken is otherwise just a client-suppliable value
// riding along on an unauthenticated @Public() route. Uses a real JwtService
// (not mocked) so the test exercises real sign/verify semantics, not just
// "was verifyAsync called" — everything else PublicService depends on is
// untouched by this code path, so it's stubbed rather than fully mocked,
// same convention as this codebase's other service.spec.ts files.
...
describe('PublicService preview-auth bypass (listOutlets)', () => {
  ...
  it('rejects a valid staff token belonging to a DIFFERENT shop (the property that actually matters)', async () => {
    const db = createMockDb();
    ...
    db.query
      ...
      .mockResolvedValueOnce([{ shopId: 2 }]); // user 99 belongs to shop 2, not 1
    const { service } = buildService(db);
  ...
  it("accepts a valid staff token belonging to THIS shop's own unpublished shop", async () => {
    const db = createMockDb();
    const { service, jwtService } = buildService(db);
    const token = await jwtService.signAsync({ sub: 5, typ: 'staff' });

    db.query
      .mockResolvedValueOnce([UNPUBLISHED_SHOP_ROW]) // resolveShop -> shop id 1
      .mockResolvedValueOnce([{ shopId: 1 }]) // user 5 belongs to shop 1 — match
  ...
  it('rejects a customer token even for the right shop (typ must be staff)', async () => {
    const db = createMockDb();
    const { service, jwtService } = buildService(db);
    ...
    await expect(
      service.listOutlets('unpublished-shop', customerToken),
    ).rejects.toThrow(NotFoundException);
  });
});
