...
// 20260816130000_fix_notification_text_column; contactNumbers/colors since
// 20260816140000_fix_contact_numbers_colors_columns) — don't "fix" that by
// passing the raw array/object straight through as a query param instead.
// Confirmed empirically against this DB, for both an array and an object
// value: mysql2 splats a raw array parameter into multiple bound values
// ("Column count doesn't match value count"), it does not JSON-encode it.
// JSON.stringify()-then-pass-as-string is correct and necessary here — a
// real MySQL JSON column parses a JSON-text string value back into the
// document it represents at insert time (confirmed: JSON_TYPE() reports
// ARRAY/OBJECT, not STRING), so this was never the bug for any of the
// three. The bug was purely each column's LONGTEXT type providing no such
// parsing at all, on either the write or read side.
