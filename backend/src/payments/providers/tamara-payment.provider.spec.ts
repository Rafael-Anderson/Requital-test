  ...
  describe('createCheckoutSession', () => {
    it('returns Tamara-provided checkout_url as the checkout URL', async () => {
      ...
      const session = await provider.createCheckoutSession({
        ...
        credentials: { apiToken: 'tok_test', apiUrl: 'https://api-sandbox.tamara.co' },
      });
    ...
    it('falls back to the platform-level env vars, including the default sandbox API URL, when unset', async () => {
      ...
      global.fetch = jest.fn().mockResolvedValue({
        ok: true,
        json: async () => ({ order_id: 'o', checkout_id: 'c', checkout_url: 'https://x' }),
      }) as unknown as typeof fetch;
  ...
  describe('parseWebhookEvent', () => {
    ...
    it('a validly-signed order_approved event confirms the order', () => {
      ...
      const payload = Buffer.from(
        JSON.stringify({ order_id: 'order_1', order_reference_id: '55', event_type: 'order_approved' }),
      );
    ...
    it('order_declined and order_expired both cancel a still-pending order', () => {
      ...
      for (const event_type of ['order_declined', 'order_expired']) {
        const payload = Buffer.from(
          JSON.stringify({ order_id: 'order_2', order_reference_id: '55', event_type }),
        );
        const result = provider.parseWebhookEvent(payload, sign(payload, secret), secret);
    ...
    it('a tampered signature is rejected — the payload is never parsed', () => {
      const provider = new TamaraPaymentProvider();
      const payload = Buffer.from(
        JSON.stringify({ order_id: 'o', order_reference_id: '1', event_type: 'order_approved' }),
      );
      expect(provider.parseWebhookEvent(payload, 'not-the-real-signature', secret)).toBeNull();
    });
    ...
    it('a signature computed with the wrong secret is rejected', () => {
      ...
      expect(
        provider.parseWebhookEvent(payload, sign(payload, 'wrong-secret'), secret),
      ).toBeNull();
    });
    ...
    it('an unrecognized event type is a safe no-op', () => {
      const provider = new TamaraPaymentProvider();
      const payload = Buffer.from(
        JSON.stringify({ order_id: 'o', order_reference_id: '1', event_type: 'order_canceled' }),
      );
