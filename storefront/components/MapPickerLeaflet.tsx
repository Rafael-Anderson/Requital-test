...
// (a brand-new address); never sent anywhere as a real value.
const DEFAULT_CENTER: [number, number] = [25.2048, 55.2708];

// <MapContainer center> only sets the *initial* view (react-leaflet docs) —
// it doesn't track further prop changes, so a search result, saved-address
// selection, or geolocation fix needs this imperative escape hatch to
...
export default function MapPickerLeaflet({ latitude, longitude, onPick, shopSlug, className = "" }: MapPickerProps) {
  ...
  async function handleSearch() {
    ...
    setSearching(true);
    try {
      const result = await geocode(shopSlug, query);
      onPick({ latitude: result.latitude, longitude: result.longitude }, result.displayName);
    } catch {
      // Same non-blocking behavior as the pre-map search box — a failed
      // lookup just doesn't move the pin, it's never surfaced as a crash.
    } finally {
      setSearching(false);
    }
  }
  ...
  function handleKeyDown(e: KeyboardEvent<HTMLInputElement>) {
    ...
    // checkout/address <form> at every call site, and HTML doesn't allow
    // nested <form> elements (Next.js flags it as a hydration error, and a
    // nested form's Enter-to-submit silently routes to the wrong form).
    // Enter-to-search is reproduced by hand instead.
    if (e.key === "Enter") {
      e.preventDefault();
      handleSearch();
    }
    ...
    <div className={className}>
      <div className="flex gap-2 mb-2">
        <input
          ...
          className="w-full h-9 rounded-lg border border-black/15 dark:border-white/15 bg-white dark:bg-zinc-900 px-3 text-sm shadow-sm shadow-black/5 outline-none transition-shadow focus:border-accent focus:ring-[3px] focus:ring-accent/20"
        />
        <button
          type="button"
        ...
        <Marker
          ...
          eventHandlers={{
            dragend: () => {
              ...
              onPick(coords, null);
              reverseGeocode(shopSlug, lat, lng)
                .then((result) => onPick(coords, result.displayName))
                .catch(() => {});
            },
          }}
        />
