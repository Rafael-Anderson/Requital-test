...
import { getShop, listCategories, listProducts } from "@/lib/api";
...
// segment, so each shop needs its own detailed sitemap (home + categories +
...
export async function GET(_request: Request, { params }: { params: Promise<{ shop: string }> }) {
  ...
  const [categories, products] = await Promise.all([
    listCategories(shopSlug).catch(() => []),
    listProducts(shopSlug).catch(() => []),
  ]);

  const urls = [
    base,
    ...categories.map((c) => `${base}?category=${c.id}`),
