...
export interface AnimationSettings {
  ...
  // add-to-cart animation. Fires only when `addToCart` (master switch) is
  // also true. See lib/fly-to-cart.tsx.
...
// .primary renders anywhere today (Hero CTA, Newsletter submit).
...
export interface ProductCardSettings {
  ...
  // §8.13.C item 4 — a one-shot animation on the wishlist heart when it's
  // toggled ON. Absent/"none" ⇒ today's plain colour swap. `pop`/`burst` are
  // built; `sweep` is reserved in the schema but not implemented yet.
